...
export const getOfwMasterlist = async (req, res) => {
  try {
    const [rows] = await pool.query(`
      ...
      WHERE sc.classification_code = 'OFW'
      ORDER BY 
          p.last_name,
          p.first_name,
          p.middle_name,
    ...
    console.error('Error fetching ofw masterlist data:', error);
    res.status(500).json({ 
      success: false, 
      message: 'Error fetching ofw masterlist data', 
      error: error.message 
    });
  }
}
