export const generateSeniorId = async (connection) => {
  ...
  try {
    ...
    const [rows] = await connection.query(
      `SELECT 
      MAX(CAST(SUBSTRING_INDEX(senior_citizen_id, '-', -1) AS UNSIGNED)) as maxId 
      FROM senior_citizen_id_applications 
      WHERE senior_citizen_id LIKE CONCAT('SC-', ?, '-%')`,
      [`${datePrefix}%`]
    );
    ...
    const [existingCheck] = await connection.query(
      `SELECT COUNT(*) as count FROM senior_citizen_id_applications WHERE senior_citizen_id = ?`,
      [surveyId]
    );
